            ...
            steps {
                script {
                    def artifactExists = sh(
                        script: '''
                            curl -s -o /dev/null -w "%{http_code}" -u admin:admin "http://192.167.33.10:8081/repository/maven-public/tn/esprit/spring/kaddem/0.0.1-SNAPSHOT/4TWIN2_G3_kaddem.jar"
                        ''',
                        returnStdout: true
                    ).trim()
